...
async def send_chat_message(payload: ChatMessageRequest):
    """
    Processes customer chat messages for StrideHub Shoes:
    ...
    4. Validates & sanitizes copy against Firebase facts.
    5. Syncs state to Firestore and returns rich product/order payloads.
    """
    ...
    order_info = None
    order_match = re.search(r'#?(SH-\d{4}|\d{4})', message_text, re.IGNORECASE)
    if "order" in message_text.lower() and order_match:
        ord_id = order_match.group(1).upper()
        if not ord_id.startswith("SH-"):
            ord_id = f"SH-{ord_id}"
    ...
    if not matched_products and any(cat in message_text.lower() for cat in ["running", "casual", "formal", "trail", "sneaker", "walking"]):
        for cat in ["running", "casual", "formal", "trail", "sneaker", "walking"]:
    ...
    # 5. Prepare and Execute LangGraph State Machine
    ...
    reply_text = final_state.get("reply_text", "How can I help you with your shoe search today?")
    ...
    # If order tracking info was found, append natural tracking context
    if order_info:
        reply_text = f"Order #{order_info['order_id']} ({order_info['product_name']}) is currently {order_info['status'].upper()}. Courier: {order_info.get('courier_partner', 'BlueDart')} (Tracking: {order_info.get('tracking_id', 'N/A')}). Estimated delivery: {order_info.get('estimated_delivery', '3-4 days')}."

    # 6. Save updated lead state & assistant reply in Firestore
    ...
    # 7. Generate contextual Quick Reply chips
    quick_replies = []
