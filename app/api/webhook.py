...
async def verify_webhook(
    ...
    Validates hub.verify_token and responds with the hub.challenge string.
    """
...
async def receive_webhook(
    ...
    4. Retrieves context and conversation history from Firebase.
    5. Executes LangGraph workflow (Gemini AI + Firebase data grounding).
    6. Dispatches reply to WhatsApp user.
    """
    ...
    try:
        ...
        # Fetch recent conversation history from Firebase for memory context
        recent_history = firebase_service.get_recent_conversation_history(
        ...
        initial_state: SalesAgentState = {
            ...
            "lead_id": str(lead.id),
            "contact_number": contact_number,
            "lead_name": lead.name or contact_name,
            "current_stage": lead.stage,
            "incoming_message": message_text,
            "need_summary": (lead.metadata_json or {}).get("need_summary"),
        ...
        # Graceful fallback response so webhook never crashes
        return {
            "status": "error_handled",
            "reason": "Temporary processing issue handled gracefully",
            "reply_sent": "Sorry, I’m having a small issue processing that right now. Please try again in a moment."
        }
